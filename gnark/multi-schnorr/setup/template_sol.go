...
const multischnorrTemplate = `// SPDX-License-Identifier: MIT

pragma solidity ^0.8.0;
import "@openzeppelin/contracts/access/Ownable.sol";

/// @title Groth16 verifier for Multischnorr.
...
contract MultischnorrVerifier is Ownable {
    ...
    /// @param _newThreshold New minimum number of required signatures
    ...
    /// @notice Update the validator set merkle root (only owner)
    /// @param _newMerkleRoot New merkle root of validator public keys
    function updateMerkleRoot(uint256 _newMerkleRoot) external onlyOwner {
        require(_newMerkleRoot != 0, "Merkle root cannot be zero");
        merkleRoot = _newMerkleRoot;
        emit MerkleRootUpdated(merkleRoot, _newMerkleRoot);
    }
